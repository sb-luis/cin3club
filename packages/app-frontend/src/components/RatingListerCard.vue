...
<template>
  <div
    @click="() => mainStore.navigate({ path: `${rating.mediaItem.mediaType}/${rating.mediaItem.tmdbId}` })"
    v-if="rating.mediaItem"
  ...
  >
    <div class="max-w-[50%] text-start md:max-w-[480px]">
      <div class="pb-2">
        <p class="break-words text-2xl md:text-3xl">{{ rating.mediaItem.title }}</p>
        <p
          v-if="rating.mediaItem.originalTitle !== rating.mediaItem.title"
          class="text-primary-900 pt-1 text-sm italic md:text-xl"
        >
          {{ rating.mediaItem.originalTitle }}
        </p>
      </div>
      <div class="flex items-center space-x-2 md:space-x-4 md:text-lg">
        <p>{{ dateStrToYear(rating.mediaItem.releaseYear) }}</p>
      </div>
    ...
    <div>
      <img
        :src="fullPosterUrl(rating.mediaItem.posterPath)"
